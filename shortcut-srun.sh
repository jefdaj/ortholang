...
# TODO so is srun changing the current directory? that would explain not finding the db
...
case "$(basename "$BIN")" in
	crb-blast) echo "found crb-blast"; ARGS="$ARGS --split --threads=$SLURM_CPUS_PER_TASK" ;;
esac
