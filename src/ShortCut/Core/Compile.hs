...
cList s e@(CutList rtn _ _ exprs) -- TODO should pass whole list here?
  | rtn `elem` [ListOf str, ListOf num] = cListOne s e
...
-- TODO what if the lists aren't literal? like, they're generated by a fn call?
--      (could use .txt for the literal ones I guess?)
cListOne :: (CutScript, CutConfig) -> CutExpr -> Rules ExprPath
cListOne (_,cfg) e@(CutList rtn _ _ exprs) = do
-- cListOne rtn (_,cfg) exprs = do
  let lits  = sort $ map extractLit exprs
  lits' <- liftIO $ mapM absolutize lits

  let (ExprPath outPath) = exprPath cfg e [] -- TODO only depend on final exprs

  liftIO $ putStrLn $ "lits': " ++ show lits'
  liftIO $ putStrLn $ "cListOne outPath: " ++ outPath
  outPath %> \_ -> do
...
cListMany s@(_,cfg) e@(CutList _ _ _ exprs) = do
  paths <- mapM (cExpr s) exprs

  -- TODO only depend on final exprs
  let (ExprPath outPath) = exprPath cfg e [] -- TODO why the paths here?
      -- paths' = map (\(ExprPath p) -> makeRelative (cfgTmpDir cfg) p) paths
  liftIO $ putStrLn $ "cListMany outPath: " ++ outPath
  outPath %> \_ -> do
