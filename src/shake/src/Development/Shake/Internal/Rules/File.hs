    ...
    let run o@(FileQ x) oldBin@(fmap getEx -> old) dirty = do
            ...
            let rebuild = do
                    ...
                    act <- case userRuleMatch rules $ \(FileRule f) -> f x of
												...
												-- Modified by Jeff to silently use the first rule, because in ShortCut all duplicates should be
												-- equivalent. Yes it's a horrible hack, but the alternative was to make a thread-safe set of paths
												-- used and pass it around everywhere to check whether each rule is a duplicate before adding it.
                        -- [r] -> return $ Just r
