...
-- TODO make an empty OrthoLangState so you can run these in ghci again
...
parseWithLeftOver :: ParseM a -> OrthoLangState -> String -> Either ParseError (a,String)
parseWithLeftOver p s = runParseM ((,) <$> p <*> leftOver) s
  where
    leftOver = manyTill anyToken eof

parseWithEof :: ParseM a -> OrthoLangState -> String -> Either ParseError a
