...
{-|
The main entry point for compiling any 'OrthoLangExpr'. Use this by default
...
rExpr' s e@(OrthoLangList _ _ _ _   ) = rSourceList s e -- TODO deprecate this?
...
These use the 'OrthoLangList' constructor; generated lists come as 'OrthoLangFun's.

TODO remove the insert digests hack
-}
rSourceList :: RulesFn
rSourceList s e@(OrthoLangList rtn _ _ es)
  | rtn `elem` [Empty, str, num] = withInsertNewRulesDigests s (e:es) $ rSourceListLits  s e
  | otherwise                    = withInsertNewRulesDigests s (e:es) $ rSourceListPaths s e
rSourceList _ _ = error "bad arguemnt to rSourceList"
...
actual arguments are added to their 'hExprs' entry at the same time. Note this
is different from how function-generated lists of literals (or paths) are
handled, because their arguments won't be known until after the function runs.

TODO can it be mostly unified with rSourceListPaths digest-wise?

TODO what happens when you make a list of literals in two steps using links?
-}
rSourceListLits :: RulesFn
rSourceListLits s@(_, cfg, ref, ids) e@(OrthoLangList _ _ _ exprs) = do
  ...
  where
    ...
    outPath' = debugRules cfg "rSourceListLits" e $ fromOrthoLangPath cfg outPath
rSourceListLits _ e = error $ "bad argument to rSourceListLits: " ++ show e
...
-}
rSourceListPaths :: RulesFn
rSourceListPaths s@(_, cfg, ref, ids) e@(OrthoLangList rtn salt _ exprs) = do
  ...
  let paths'   = map (\(ExprPath p) -> toOrthoLangPath cfg p) paths
      ...
      outPath  = exprPath s e
      outPath' = debugRules cfg "rSourceListPaths" e $ fromOrthoLangPath cfg outPath
  outPath' %> \_ -> aSourceListPaths cfg ref ids paths' outPath
  return (ExprPath outPath')
rSourceListPaths _ _ = error "bad arguemnts to rSourceListPaths"
