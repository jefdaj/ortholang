...
pList = debugParser "pList" $ do
  ...
  case eType of
    Left err -> fail err
    Right t  -> return $ CutList t (RepeatSalt 0) deps terms
...
pBop' bop e1 e2 = case (fTypeCheck bop) [typeOf e1, typeOf e2] of
  ...
  Right rtn -> CutBop rtn (RepeatSalt 0) (union (depsOf e1) (depsOf e2)) (fName bop) e1 e2
...
typecheckArgs fn args = case (fTypeCheck fn) (map typeOf args) of
  ...
  Right rtn -> let deps = foldr1 union $ map depsOf args
               in return $ CutFun rtn (RepeatSalt 0) deps (fName fn) args
