...
rOrthologFilterStr fnName pickerFn st@(_, cfg, ref, _) e@(CutFun _ _ _ _ [groupLists, idLists]) = do
  ...
  out' %> \_ -> do
    ...
    need' cfg ref "shortcut.modules.orthogroups.rOrthologFilterStr" [ogsPath, idsPath] -- TODO shouldn't cmdInPatterns pick that up?
...
rOrthologFilterStrFrac fnName pickerFn st@(_, cfg, ref, _) e@(CutFun _ _ _ _ [frac, groupLists, idLists]) = do
  ...
  out' %> \_ -> do
    ...
    need' cfg ref "shortcut.modules.orthogroups.rOrthologFilterStrFrac" [ogsPath, idsPath] -- TODO shouldn't cmdInPatterns pick that up?
    runCmd cfg ref $ CmdDesc
      { cmdParallel = False
      , cmdFixEmpties = True
      , cmdOutPath = out'
