...
cCombos comboFn s@(_,cfg) expr@(CutFun _ _ _ fnName [iList]) = do
  ...
  let oType = ListOf $ typeOf iList
      (ExprPath oList) = exprPathTyped cfg oType expr [ExprPath iPath] -- TODO need fnName too like before?
  oList %> \out -> do
    need [iPath]
    elements <- fmap lines $ readFile' iPath
    let combos = comboFn elements
        lType  = typeOf iList -- TODO is this right?
        oPaths = map (\(ExprPath p) -> p) $ map (\e -> exprPathTyped cfg lType iList [ExprPath out, ExprPath e]) elements
