...
pList = debugParser "pList" $ do
  ...
  case eType of
    Left err -> fail err
    Right t  -> return $ RrrList t 0 deps terms
...
operatorTable :: RrrConfig -> [[E.Operator String RrrState Identity RrrExpr]]
...
pBop bop
  | fFixity bop == Infix = (debugParser ("pBop " ++ fName bop) (pSym (head $ fName bop))) *> (return $ \e1 e2 ->
    ...
    in case (fTypeCheck bop) [typeOf e1, typeOf e2] of
      ...
      Right rtn -> RrrBop rtn 0 deps (fName bop) e1 e2)
...
pFunArgs :: String -> [RrrExpr] -> ParseM RrrExpr
...
pRef = debugParser "pRef" $ do
  ...
  case lookup v scr of
    ...
    Just e -> return $ RrrRef (typeOf e) 0 (depsOf e) v

-- pFunOrRef :: ParseM RrrExpr
-- pFunOrRef = debugParser "pFunOrRef" $ do
--   name <- try pIden -- TODO when it gets here, loader is rrr off to oader
...
typecheckArgs :: RrrFunction -> [RrrExpr] -> ParseM RrrExpr
typecheckArgs fn args = case (fTypeCheck fn) (map typeOf args) of
  Left  msg -> error msg
  Right rtn -> let deps = foldr1 union $ map depsOf args
               in return $ RrrFun rtn 0 deps (fName fn) args

-----------------
-- expressions --
-----------------

pParens :: ParseM RrrExpr
