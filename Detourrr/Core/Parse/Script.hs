...
pResult :: ParseM RrrAssign
pResult = debugParser "pResult" $ do
  -- (_, cfg, _) <- getState
  e <- pExpr
  -- let e' = debugParser cfg "pResult" e
  return (RrrVar "result", e)

pStatement :: ParseM RrrAssign
...
-- TODO need RrrState here? or just RrrConfig?
parseStatement :: RrrState -> String -> Either ParseError RrrAssign
...
-- TODO clarify that
parseString :: RrrConfig -> Locks -> HashedSeqIDsRef -> String
            -> Either ParseError RrrScript
...
parseFile :: RrrConfig -> Locks -> HashedSeqIDsRef -> FilePath
          -> IO (Either ParseError RrrScript)
...
parseFileIO :: RrrConfig -> Locks -> HashedSeqIDsRef -> FilePath -> IO RrrScript
