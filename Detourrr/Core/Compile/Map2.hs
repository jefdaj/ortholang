module Detourrr.Core.Compile.Map2
...
{- Haskell functions for making mapped versions of Detourrr functions.
 ...
 - a higher-order RulesFn for use in the fRules field of a RrrFunction.
 -}

import Development.Shake
import Detourrr.Core.Types
import Detourrr.Core.Paths
import Detourrr.Core.Util (digest)
import Control.Monad (forM, forM_)
import Detourrr.Core.Actions (readStrings, writeStrings, debugL)
...
{- These take explicit path arguments rather than a [RrrPath] in order to be
...
map1of1 inType outType act1 cfg locks ids out a1 = do
  inPaths <- readStrings inType cfg locks $ fromRrrPath cfg a1
  ...
  outPaths <- forM inPaths $ \i -> do
    let o = tmpDir </> digest [out, toRrrPath cfg i] <.> extOf outType
    debugL cfg $ "map1of1 o: " ++ show o
    act1 cfg locks ids (toRrrPath cfg o) (toRrrPath cfg i)
    return o
  debugL cfg $ "map1of1 outPaths: " ++ show outPaths
  debugL cfg $ "map1of1 out: " ++ show out
  writeStrings outType cfg locks (fromRrrPath cfg out) outPaths

map1of2 :: RrrType -> RrrType -> Action2 -> Action2
map1of2 = undefined

map2of2 :: RrrType -> RrrType -> Action2 -> Action2
map2of2 = undefined

map1of3 :: RrrType -> RrrType -> Action3 -> Action3
map1of3 = undefined

-- TODO does this need to be more elaborate?
mapCache :: RrrConfig -> FilePath
...
map2of3 :: RrrType -> RrrType -> Action3 -> Action3
map2of3 inType outType act3 cfg locks ids out a1 a2 a3 = do
  inPaths <- readStrings inType cfg locks $ fromRrrPath cfg a2
  let tmpDir   = mapCache cfg
      outPaths = (flip map) inPaths $ \i ->
                   tmpDir </> digest [out, toRrrPath cfg i] <.> extOf outType
  ...
  forM_ ioPairs $ \(i,o) -> act3 cfg locks ids (toRrrPath cfg o) a1 (toRrrPath cfg i) a3
  writeStrings outType cfg locks (fromRrrPath cfg out) outPaths
...
map3of3 :: RrrType -> RrrType -> Action3 -> Action3
map3of3 = map3Base -- because it's already the 3rd

map3Base :: RrrType -> RrrType -> Action3 -> Action3
map3Base inType outType act3 cfg locks ids out a1 a2 a3 = do
  ...
  inPaths <- readStrings inType cfg locks $ fromRrrPath cfg a3

  -- but this way breaks something too, right?
  -- a3path  <- readPath cfg locks $ fromRrrPath cfg a3
  -- debugL cfg $ "map3Base a3path: " ++ show a3path
  -- inPaths <- readStrings inType cfg locks $ fromRrrPath cfg a3path

  debugL cfg $ "map3Base inPaths read from a3: " ++ show inPaths
  let tmpDir   = cfgTmpDir cfg </> "cache" </> "map" -- TODO figure this out better
      outPaths = (flip map) inPaths $ \i -> tmpDir </> digest [out, toRrrPath cfg i] <.> extOf outType
      ioPairs  = zip inPaths outPaths
  -- debugL cfg $ "map3Base outPaths: " ++ show outPaths
  -- debugL cfg $ "map3Base out: " ++ show out
  -- forM_ ioPairs $ \(i,o) -> act3 cfg locks (toRrrPath cfg o) a1 a2 (toRrrPath cfg i)
  -- TODO can this be done with forP in parallel? have to only do one overall read lock on input
  forM_ ioPairs $ \(i,o) -> do
    debugL cfg $ "map3Base input and output: " ++ show i ++ ", " ++ show o
    act3 cfg locks ids (toRrrPath cfg o) a1 a2 (toRrrPath cfg i)
  writeStrings outType cfg locks (fromRrrPath cfg out) outPaths
...
--   forM_ ioPairs $ \(i,o) -> do
--     debugL cfg $ "map3Base input and output: " ++ show i ++ ", " ++ show o
--     act3 cfg locks (toRrrPath cfg o) a1 a2 (toRrrPath cfg i)
--   writeStrings outType cfg locks (fromRrrPath cfg out) outPaths
...
-- Compile a RrrFunction with 3 arguments
...
rFun1 act1 st@(_, cfg, ref, ids) expr@(RrrFun _ _ _ _ [a1]) = do
  (ExprPath arg1') <- rExpr st a1
  let arg1   = toRrrPath cfg arg1'
      oPath  = exprPath st expr
      oPath' = debugRules cfg "rFun1" expr $ fromRrrPath cfg oPath
  oPath' %> \_ -> do
...
rFun3 act3 st@(_, cfg, ref, ids) expr@(RrrFun _ _ _ _ [a1, a2, a3]) = do
  (ExprPath arg1') <- rExpr st a1
  (ExprPath arg2') <- rExpr st a2
  (ExprPath arg3') <- rExpr st a3
  let arg1   = toRrrPath cfg arg1'
      arg2   = toRrrPath cfg arg2'
      arg3   = toRrrPath cfg arg3'
      oPath  = exprPath st expr
      oPath' = debugRules cfg "rFun3" expr $ fromRrrPath cfg oPath
  oPath' %> \_ -> do
...
singleton :: RrrExpr -> RrrExpr
singleton e = RrrList (typeOf e) (saltOf e) (depsOf e) [e]
