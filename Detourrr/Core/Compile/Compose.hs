...
compose1 :: String      -- overall function name
         ...
         -> RrrFunction -- first function (takes inputs, returns intermediate)
         -> RrrType     -- intermediate type to be passed from fn1 to fn2
         -> RrrFunction -- second function (takes intermediate, returns output)
         -> RrrFunction -- overall fn (runs fn1, then fn2 on its output)
compose1 name desc fn1 type1 fn2 = RrrFunction
  { fName      = name
...
tCompose1 :: RrrFunction -> RrrType -> RrrFunction -> TypeChecker
...
rCompose1 :: RrrFunction -> RrrType -> RrrFunction -> RulesFn
rCompose1 fn1 rtn1 fn2 st (RrrFun rtn2 salt deps _ args) = (fRules fn2) st expr2
  where
    expr1'  = RrrFun rtn1 salt deps (fName fn1) args
    expr1'' = RrrRules $ CompiledExpr expr1' $ (fRules fn1) st expr1'
    expr2   = RrrFun rtn2 salt deps (fName fn2) [expr1'']
