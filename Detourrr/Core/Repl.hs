...
runRepl :: RrrConfig -> Locks -> HashedSeqIDsRef -> IO ()
...
mkRepl :: [(String -> ReplM (Maybe String))] -> Handle
       -> RrrConfig -> Locks -> HashedSeqIDsRef -> IO ()
...
shortRrrPrompt :: RrrConfig -> String
...
loop (promptFn:promptFns) hdl = do
  st@(_, cfg, _, _)  <- get
  Just line <- promptFn $ shortRrrPrompt cfg -- TODO can this fail?
...
step :: RrrState -> Handle -> String -> IO RrrState
...
runStatement :: RrrState -> Handle -> String -> IO RrrState
...
runCmd :: RrrState -> Handle -> String -> IO RrrState
...
cmds :: RrrConfig -> [(String, RrrState -> Handle -> String -> IO RrrState)]
cmds cfg =
  [ ("help"     , cmdHelp    )
  , ("load"     , cmdLoad    )
...
cmdHelp :: RrrState -> Handle -> String -> IO RrrState
...
cmdLoad :: RrrState -> Handle -> String -> IO RrrState
...
depsOnly :: RrrExpr -> RrrScript -> RrrScript
depsOnly expr scr = deps ++ [res]
  where
    deps = filter (\(v,_) -> (elem v $ depsOf expr)) scr
    res  = (RrrVar "result", expr)

-- TODO where should this go?
saveScript :: RrrScript -> FilePath -> IO ()
...
cmdNeededBy :: RrrState -> Handle -> String -> IO RrrState
cmdNeededBy st@(scr, cfg, _, _) hdl var = do
  case lookup (RrrVar var) scr of
    Nothing -> hPutStrLn hdl $ "Var '" ++ var ++ "' not found"
    -- Just e  -> prettyAssigns hdl (\(v,_) -> elem v $ (RrrVar var):depsOf e) scr
    Just e  -> pPrintHdl cfg hdl $ filter (\(v,_) -> elem v $ (RrrVar var):depsOf e) scr
  return st

-- TODO move to Pretty.hs
-- prettyAssigns :: Handle -> (RrrAssign -> Bool) -> RrrScript -> IO ()
-- prettyAssigns hdl fn scr = do
  -- txt <- renderIO $ pPrint $ filter fn scr
  -- hPutStrLn hdl txt

cmdNeeds :: RrrState -> Handle -> String -> IO RrrState
cmdNeeds st@(scr, cfg, _, _) hdl var = do
  let var' = RrrVar var
  case lookup var' scr of
    Nothing -> hPutStrLn hdl $ "Var '" ++ var ++ "' not found"
    Just _  -> pPrintHdl cfg hdl $ filter (\(v,_) -> elem v $ (RrrVar var):rDepsOf scr var') scr
  return st

-- TODO factor out the variable lookup stuff
cmdDrop :: RrrState -> Handle -> String -> IO RrrState
...
cmdType :: RrrState -> Handle -> String -> IO RrrState
...
showExprType :: RrrState -> String -> String
showExprType st e = case parseExpr st e of
  Right expr -> show $ typeOf expr
  Left  err  -> show err

showAssignType :: RrrAssign -> String
showAssignType (RrrVar v, e) = unwords [typedVar, "=", prettyExpr]
  where
...
-- TODO factor out the variable lookup stuff
cmdShow :: RrrState -> Handle -> String -> IO RrrState
...
cmdQuit :: RrrState -> Handle -> String -> IO RrrState
cmdQuit _ _ _ = throw QuitRepl
-- cmdQuit _ _ _ = ioError $ userError "Bye for now!"

cmdBang :: RrrState -> Handle -> String -> IO RrrState
...
cmdConfig :: RrrState -> Handle -> String -> IO RrrState
...
quotedCompletions :: MonadIO m => RrrState -> String -> m [Completion]
...
nakedCompletions :: MonadIO m => RrrState -> String -> String -> m [Completion]
nakedCompletions (scr, cfg, _, _) lineReveresed wordSoFar = do
  ...
  where
    ...
    varNames = map ((\(RrrVar v) -> v) . fst) scr
