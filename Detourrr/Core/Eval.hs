...
prettyResult cfg ref (ScoresOf _)  f = do
  s <- liftIO (defaultShow cfg ref $ fromDtrPath cfg f)
  return $ text s
...
eval :: Handle -> DtrConfig -> Locks -> HashedSeqIDsRef -> DtrType -> Rules ResPath -> IO ()
  ...
  where
    ...
    eval' rpath = myShake cfg $ do
      ...
      "eval" ~> do
        ...
        res  <- prettyResult cfg ref rtype $ toDtrPath cfg path
