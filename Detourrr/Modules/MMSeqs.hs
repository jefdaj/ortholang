...
dtrModule = DtrModule
  ...
  , mFunctions =
      ...
      , mmseqsSearchDb
      , mmseqsSearch
      , mmseqsDbLength -- TODO write this, TODO unify with length
      ]
  }
...
aMmseqsSearchDb :: DtrConfig -> Locks -> FilePath -> FilePath -> FilePath -> FilePath -> Action ()
...
rMmseqsSearch st (DtrFun rtn salt deps name [e, q, s])
  ...
  where
    ...
    sDb = DtrFun mms salt (depsOf s) "mmseqs_createdb" [s] -- TODO also accept a fa.list here?
...
mmseqsDbLength :: DtrFunction
mmseqsDbLength = let name = "mmseqs_dblength" in DtrFunction
  { fName      = name
  , fTypeDesc  = name ++ " : mms -> num"
  , fTypeCheck = tMmseqsSearch name
  , fDesc      = Just "Get the number of sequences (or overall size?) of an MMseqs2 database"
  , fFixity    = Prefix
  , fRules     = rMmseqsDbLength
  }

rMmseqsDbLength :: RulesFn
rMmseqsDbLength = undefined
